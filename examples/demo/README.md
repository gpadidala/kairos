# KAIROS — Self-Contained Local Demo

Spin up KAIROS + Grafana + Mimir with synthetic AKS-style metrics — **no cluster required**.
...
- **Grafana** — pre-provisioned Mimir datasource + two KAIROS dashboards.
...
- **KAIROS** — runs in approval mode with UI enabled; discovers the 4 demo workloads from a static YAML (no k8s API access needed).
...
3. KAIROS container reports `audit_db_ready` + `kairos_starting`.

Then open:

| URL | What you'll see |
|---|---|
| **http://localhost:8090/ui** | KAIROS UI — dashboard, pending approvals, history, KEDA activity, alerts |
| http://localhost:8090/docs | Swagger UI for the JSON API |
| http://localhost:3000 | Grafana (admin/admin) — KAIROS folder has 2 dashboards |
...
```bash
# Trigger a pipeline run (KAIROS fetches metrics, forecasts, emits decisions)
...
The `seed` container pushes metrics that deliberately trigger different KAIROS rules:

| Workload | Namespace | Runtime | Scenario | KAIROS decision |
|---|---|---|---|---|
...
# Edit docker-compose.yaml — under the kairos service environment:
#   KAIROS_FEATURES__ENABLE_PR_CREATION: "true"
#   KAIROS_GITHUB__REPO: "your-org/your-gitops-repo"
#   KAIROS_GITHUB__TOKEN: "${GITHUB_TOKEN}"
...
Use `examples/demo/gitops-repo/` as the template for your GitOps repo layout — it includes the `.github/workflows/validate.yml` that CI-gates every PR KAIROS opens.
...
This is what a real team's app looks like with KAIROS annotations wired in. Any of the three formats can be dropped into your GitOps repo; KAIROS handles both Kustomize and Helm edits via ruamel.yaml round-trip.
...
- **Port conflicts** — 3000 (Grafana), 8090 (KAIROS), 9009 (Mimir) are the exposed ports. Edit `docker-compose.yaml` if any are taken.
