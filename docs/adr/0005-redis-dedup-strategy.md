...
## Context
KAIROS runs every 30 minutes. A given workload's forecast + decision may be identical across many consecutive runs (capacity trends are slow). Without deduplication we would:
...
- TTLs are configurable via `KAIROS_REDIS__DEDUP_TTL_*_SECONDS`.
- A dedup hit is **not** an error — it's logged at INFO, counted in `kairos_dedup_hits_total{kind}`, and returns a `PRResult`/`NotificationResult` with `dedup_hit=True`.
...
- The dedup hash is stable across KAIROS restarts, giving us idempotency guarantees across pods and rolling deployments.
