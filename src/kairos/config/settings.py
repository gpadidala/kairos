"""Pydantic-settings configuration. All env-driven, prefix KAIROS_, nested via `__`."""
...
class GitHubSettings(BaseModel):
    ...
    labels: list[str] = Field(default_factory=lambda: ["kairos", "autoscaling"])
    api_url: HttpUrl = HttpUrl("https://api.github.com")


class GrafanaSettings(BaseModel):
    # Server-side URL — what the KAIROS container hits for API calls (in-cluster
...
class LLMSettings(BaseModel):
    ...
    ollama: LLMProviderConfig = LLMProviderConfig(
        model="llama3.1:8b", base_url=HttpUrl("http://ollama.kairos.svc:11434")
    )


class RedisSettings(BaseModel):
    url: str = "redis://redis.kairos.svc:6379/0"
...
class AuditDBSettings(BaseModel):
    ...
    url: str = "sqlite+aiosqlite:///./kairos-audit.db"
    echo: bool = False
...
class SMTPSettings(BaseModel):
    ...
    from_addr: str = "kairos@example.com"
...
class Settings(BaseSettings):
    """Root settings. Env prefix KAIROS_. Nested via `__`."""
