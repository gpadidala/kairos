"""Build parameterized Grafana dashboards for KAIROS.

Two dashboards ship:
- `kairos-platform` — KAIROS's self-observability (wired from static JSON file)
- `kairos-predictions` — per-workload forecast overlay + risk window
...
file under `deploy/grafana/dashboards/kairos-platform.json`.
"""
...
def build_predictions_dashboard(
    *,
    ...
    return {
        "uid": "kairos-predictions",
        "title": title,
        "schemaVersion": 39,
        "version": 0,
        "editable": True,
        "tags": ["kairos", "forecast", "capacity"],
        "timezone": "utc",
        ...
        "annotations": {
            "list": [
                {
                    "name": "KAIROS decisions",
                    "datasource": {"type": "prometheus", "uid": datasource_uid},
                    "enable": True,
                    "iconColor": "rgba(255, 96, 96, 1)",
                    "target": {
                        "expr": "kairos_decisions_total > 0",
                        "queryType": "",
                    },
                }
            ]
