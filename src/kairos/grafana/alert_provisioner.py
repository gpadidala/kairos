...
def _rule_uid(workload: Workload, metric: str) -> str:
    ...
    return "kairos-" + hashlib.sha1(raw, usedforsecurity=False).hexdigest()[:16]
...
def build_cpu_rule(
    *,
    ...
    return {
        "uid": uid,
        "title": f"KAIROS CPU pressure: {workload.uid}",
        "ruleGroup": f"kairos-{workload.namespace}",
        ...
        "annotations": {
            ...
            "runbook_url": "https://example.com/runbooks/kairos-cpu",
        },
