...
from pcap.config.settings import SMTPSettings
from pcap.domain.enums import NotificationChannel
from pcap.domain.models import NotificationResult
from pcap.notify.base import NotificationPayload, Notifier
from pcap.observability.metrics import NOTIFICATIONS_SENT
...
def build_email(payload: NotificationPayload) -> tuple[str, str, str]:
    ...
    d = payload.decision
    w = d.workload
    subject = f"[PCAP] {d.action.value} for {w.uid}"
...
<html><body style="font-family:system-ui,-apple-system,sans-serif;">
  <h2>PCAP: {d.action.value} for <code>{w.uid}</code></h2>
  <p>{d.rationale}</p>
  ...
  <small>Correlation ID: <code>{d.correlation_id}</code></small>
    ...
    plain_lines = [
        f"PCAP: {d.action.value} for {w.uid}",
        "",
        d.rationale,
        "",
